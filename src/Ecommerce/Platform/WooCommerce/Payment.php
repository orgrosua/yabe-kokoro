...
class Payment
{
    ...
    {
        return (bool) \get_post_meta($post_id, 'kokoro_renewal_complete', \true);
    }
    private function is_should_generate(int $post_id) : bool
    {
        return (bool) \get_post_meta($post_id, 'kokoro_should_generate', \true);
    }
    private function is_should_renewal(int $post_id) : bool
    {
        return (bool) \get_post_meta($post_id, 'kokoro_should_renewal', \true);
    }
    private function number_active_duration(int $post_id) : int
    {
        return (int) \get_post_meta($post_id, 'kokoro_active_duration', \true);
    }
    private function number_max_sites(int $post_id) : int
    {
        return (int) \get_post_meta($post_id, 'kokoro_max_sites', \true);
    }
    ...
    private function refresh_order(int $order_id) : void
    {
        ...
        foreach ($licenseOrders as $licenseOrder) {
            ...
            $wpdb->update(\sprintf('%s%s_licenses', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['status' => \true], ['id' => $row->id], ['%d'], ['%d']);
        }
    }
    ...
    private function revoke(int $payment_id, int $order_id, int $license_id)
    {
        ...
        \delete_post_meta($payment_id, 'kokoro_order_complete');
    }
    private function revoke_license(int $license_id)
    {
        /** @var wpdb $wpdb */
        global $wpdb;
        $wpdb->delete(\sprintf('%s%s_licenses', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['id' => $license_id], ['%d']);
    }
    private function revoke_order(int $order_id)
    {
        /** @var wpdb $wpdb */
        global $wpdb;
        $wpdb->delete(\sprintf('%s%s_orders', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['id' => $order_id, 'vendor' => 'woocommerce'], ['%d', '%s']);
    }
    private function deactivate(int $order_id, int $license_id, string $new_status)
    {
        /** @var wpdb $wpdb */
        global $wpdb;
        $wpdb->update(\sprintf('%s%s_licenses', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['status' => \false], ['id' => $license_id], ['%d'], ['%d']);
        $wpdb->update(\sprintf('%s%s_orders', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['order_status' => $new_status], ['id' => $order_id, 'vendor' => 'woocommerce'], ['%s'], ['%d', '%s']);
    }
    ...
    private function when_subscription_status_completed(WC_Subscription $wcSubscription, string $old_status, string $new_status) : void
    {
        ...
        foreach ($order->get_items() as $orderItem) {
            ...
            foreach ($licenseOrders as $licenseOrder) {
                ...
                $active_duration = $this->number_active_duration((int) $product->get_id());
                $wpdb->update(\sprintf('%s%s_licenses', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['expired_at' => $active_duration > 0 ? \strtotime(\sprintf('+%d days', $active_duration), $expired_at) : null], ['id' => $row->id], ['%d'], ['%d']);
        ...
        \update_post_meta($wcSubscription->get_id(), 'kokoro_renewal_complete', \true);
    }
}

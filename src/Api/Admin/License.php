...
class License extends AbstractApi implements ApiInterface
{
    ...
    private function store(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $wpdb->insert(\sprintf('%s%s_licenses', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['uid' => Common::random_slug(10), 'title' => $title, 'license_key' => $license_key, 'status' => $status, 'expired_at' => $expired_at, 'max_sites' => $max_sites, 'user_id' => $user_id, 'created_at' => \time(), 'updated_at' => \time()], ['%s', '%s', '%s', '%d', '%d', '%d', '%d', '%d', '%d']);
        $id = $wpdb->insert_id;
        \do_action('a!yabe/kokoro/api/admin/license:store', $id);
    ...
    private function update_status(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        if ($count === 0) {
            return new WP_REST_Response(['message' => \__('License not found', 'yabe-kokoro')], 404, []);
        }
        $wpdb->update(\sprintf('%s%s_licenses', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['status' => $status], ['id' => $id], ['%d'], ['%d']);
        \do_action('a!yabe/kokoro/api/admin/license:update_status', $id);
    ...
    private function destroy(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $url_params = $wprestRequest->get_url_params();
        $id = (int) $url_params['id'];
        $item = $wpdb->get_row($wpdb->prepare("\n                SELECT *\n                FROM {$wpdb->prefix}{$wpdb->yabe_kokoro_prefix}_licenses\n                WHERE id = %d\n            ", $id));
        if (!$item) {
            return new WP_REST_Response(['message' => \__('License not found', 'yabe-kokoro')], 404, []);
        }
        $wpdb->delete(\sprintf('%s%s_licenses', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['id' => $id], ['%d']);
        \do_action('a!yabe/kokoro/api/admin/license:destroy', $item);
    ...
    private function update(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $max_sites = $payload['max_sites'] && (int) $payload['max_sites'] > 0 ? (int) $payload['max_sites'] : null;
        $user_id = isset($payload['user_id']) ? (int) $payload['user_id'] : null;
        $wpdb->update(\sprintf('%s%s_licenses', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['title' => $title, 'license_key' => $license_key, 'status' => $status, 'expired_at' => $expired_at, 'max_sites' => $max_sites, 'user_id' => $user_id, 'updated_at' => \time()], ['id' => $id], ['%s', '%s', '%d', '%d', '%d', '%d', '%d'], ['%d']);
        \do_action('a!yabe/kokoro/api/admin/license:update', $id);
        return new WP_REST_Response(['id' => $id], 200, []);
    }

...
class Site extends AbstractApi implements ApiInterface
{
    ...
    private function update_status(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        if ($count === 0) {
            return new WP_REST_Response(['message' => \__('Site not found', 'yabe-kokoro')], 404, []);
        }
        $wpdb->update(\sprintf('%s%s_sites', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['status' => $status], ['id' => $id], ['%d'], ['%d']);
        \do_action('a!yabe/kokoro/api/admin/site:update_status', $id);
    ...
    private function destroy(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $wpdb->delete(\sprintf('%s%s_sites', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['id' => $id], ['%d']);
        \do_action('a!yabe/kokoro/api/admin/site:destroy', $item);

...
class Remote extends AbstractApi implements ApiInterface
{
    ...
    {
        \register_rest_route(self::API_NAMESPACE, $this->get_prefix() . '/check-connection', ['methods' => WP_REST_Server::CREATABLE, 'callback' => fn(WP_REST_Request $wprestRequest): WP_REST_Response => $this->check_connection($wprestRequest), 'permission_callback' => fn(WP_REST_Request $wprestRequest): bool => $this->permission_callback($wprestRequest)]);
    ...
    private function store(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $remote_url = $payload['remote_url'];
        $wpdb->insert(\sprintf('%s%s_remotes', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['uid' => Common::random_slug(10), 'title' => $title, 'license_key' => $license_key, 'remote_url' => $remote_url, 'status' => $status, 'created_at' => \time(), 'updated_at' => \time()], ['%s', '%s', '%s', '%s', '%d', '%d', '%d']);
        $id = $wpdb->insert_id;
        \do_action('a!yabe/kokoro/api/admin/remote:store', $id);
    ...
    private function update_status(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        if ($count === 0) {
            return new WP_REST_Response(['message' => \__('Remote not found', 'yabe-kokoro')], 404, []);
        }
        $wpdb->update(\sprintf('%s%s_remotes', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['status' => $status], ['id' => $id], ['%d'], ['%d']);
        \do_action('a!yabe/kokoro/api/admin/remote:update_status', $id);
    ...
    private function destroy(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $wpdb->delete(\sprintf('%s%s_remotes', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['id' => $id], ['%d']);
        \do_action('a!yabe/kokoro/api/admin/remote:destroy', $item);
    ...
    private function update(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $remote_url = $payload['remote_url'];
        $status = (bool) $payload['status'];
        $wpdb->update(\sprintf('%s%s_remotes', $wpdb->prefix, $wpdb->yabe_kokoro_prefix), ['title' => $title, 'license_key' => $license_key, 'remote_url' => $remote_url, 'status' => $status, 'updated_at' => \time()], ['id' => $id], ['%s', '%s', '%s', '%d', '%d'], ['%d']);
        \do_action('a!yabe/kokoro/api/admin/remote:update', $id);
        return new WP_REST_Response(['id' => $id], 200, []);
    }
}

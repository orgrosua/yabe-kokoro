...
class Asset
{
    ...
    {
        if (static::$manifest === []) {
            $manifest = \file_get_contents(\dirname(YABE_KOKORO::FILE) . '/build/manifest.json');
    ...
    public static function read_entrypoints() : array
    {
        if (static::$entrypoints === []) {
            $entrypoints = \file_get_contents(\dirname(YABE_KOKORO::FILE) . '/build/entrypoints.json');
        ...
        if (isset($entrypoints[$key])) {
            ...
            if (isset($entry['js'])) {
                foreach ($entry['js'] as $js) {
                     ...
                     */
                    $dependency = \apply_filters('f!yabe/kokoro/asset:enqueue_entry.dependency', $dependency, $manifest_key, $key);
    ...
    {
        $handle = YABE_KOKORO::WP_OPTION . ':' . $key;
        ...
        if ($src === '') {
            ...
            if (isset($manifest[$key])) {
                $src = \strncmp($manifest[$key], 'http', \strlen('http')) === 0 ? $manifest[$key] : \plugins_url('build/' . $manifest[$key], YABE_KOKORO::FILE);
            } else {
                return \false;
            }
        }
        $is_registered = \wp_register_script($handle, $src, $deps, YABE_KOKORO::VERSION, $in_footer);
        if ($is_registered) {
            return $handle;
        }
        return \false;
    ...
    public static function register_style(string $key, array $deps = [], string $src = '', string $media = 'all')
    {
        ...
        $is_registered = \wp_register_style($handle, $src, $deps, YABE_KOKORO::VERSION, $media);
        if ($is_registered) {
            return $handle;
        }
        return \false;
    ...
    {
        return \plugins_url('build/', YABE_KOKORO::FILE);
    }
}

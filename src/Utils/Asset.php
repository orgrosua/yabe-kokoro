...
class Asset
{
    ...
    {
        if (static::$manifest === []) {
            $manifest = \file_get_contents(\dirname(KOKORO::FILE) . '/build/manifest.json');
    ...
    public static function read_entrypoints() : array
    {
        if (static::$entrypoints === []) {
            $entrypoints = \file_get_contents(\dirname(KOKORO::FILE) . '/build/entrypoints.json');
    ...
    public static function register_script(string $key, array $deps = [], string $src = '', bool $in_footer = \false)
    {
        ...
        if ($src === '') {
            ...
            if (isset($manifest[$key])) {
                $src = \strncmp($manifest[$key], 'http', \strlen('http')) === 0 ? $manifest[$key] : \plugins_url('build/' . $manifest[$key], KOKORO::FILE);
            } else {
                return \false;
            }
        }
        $is_registered = \wp_register_script($handle, $src, $deps, KOKORO::VERSION, $in_footer);
        if ($is_registered) {
            return $handle;
        }
        return \false;
    ...
    public static function register_style(string $key, array $deps = [], string $src = '', string $media = 'all')
    {
        ...
        $is_registered = \wp_register_style($handle, $src, $deps, KOKORO::VERSION, $media);
        if ($is_registered) {
            return $handle;
        }
        return \false;
    ...
    {
        return \plugins_url('build/', KOKORO::FILE);
    }
}
